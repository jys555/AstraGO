...
interface CardProps extends React.ComponentProps<"div"> {  hover?: boolean;  onClick?: () => void;}

function Card({ className, hover = false, onClick, children, ...props }: CardProps) {  const baseStyles = cn(    "bg-card text-card-foreground flex flex-col gap-6 rounded-xl border",    className,
  );  const hoverStyles = hover || onClick ? "cursor-pointer transition-shadow hover:shadow-lg" : "";  const content = (    <div className={cn(baseStyles, hoverStyles)} {...props}>      {children}    </div>
  );  if (onClick || hover) {  
  return (
      <motion.div        whileHover={hover || onClick ? { scale: 1.01 } : undefined}        whileTap={onClick ? { scale: 0.99 } : undefined}        onClick={onClick}      >        {content}      </motion.div>  
  );  }  return content;}
